...
parallep3=list()
Cluster3=list()
rand3.kmeans.iso=list()
rand.bagg3.kmeans.iso=list()
FM3.kmeans.iso=list()
FM.bagg3.kmeans.iso=list()
...
bagg3.kmeans.iso=parLapply(cl, Cluster3, function(x) cl_bag.LloydShapes(x, B=B, numClust=2, algorithm = "LloydShapes",  parameters = 
                                                                            list(algSteps=10,niter=10,stopCr = 1e-04,simul=FALSE, verbose=FALSE)))
...
for (c in 1:simulation) {
  ...
  rand3.kmeans.iso[[c]]=RI(bagg3.kmeans.iso[[c]]$original$asig, verd3)
  
  
  
  # Rand Index with bagging
  rand.bagg3.kmeans.iso[[c]]=RI(max.col(as.matrix(bagg3.kmeans.iso[[c]]$bagg$.Data)), verd3)
  
  
  # FMI without bagging
  FM3.kmeans.iso[[c]]=FM_index_R(bagg3.kmeans.iso[[c]]$original$asig, verd3)
  
  
  # FMI with bagging
  FM.bagg3.kmeans.iso[[c]]=FM_index_R(max.col(as.matrix(bagg3.kmeans.iso[[c]]$bagg$.Data)), verd3)
  
  # Times
  
  time.without.bagg3[[c]]=bagg3.kmeans.iso[[c]]$tempo1
  
  time.with.bagg3[[c]]=bagg3.kmeans.iso[[c]]$tempo2
  
  
}

export.kmeans.iso3=cbind(rand3.kmeans.iso,rand.bagg3.kmeans.iso, FM3.kmeans.iso,  FM.bagg3.kmeans.iso, time.without.bagg3, time.with.bagg3, time.parallelization3
)

write.table(export.kmeans.iso3, file = "kmeans_N100_iso3_B100_simul50.csv", sep = ",", 
...
parallep6=list()
Cluster6=list()
rand6.kmeans.iso=list()
rand.bagg6.kmeans.iso=list()
FM6.kmeans.iso=list()
FM.bagg6.kmeans.iso=list()
...
## Application of methods
bagg6.kmeans.iso=parLapply(cl, Cluster6, function(x) cl_bag.LloydShapes(x, B=B, numClust=2, algorithm = "LloydShapes",  parameters = 
                                                                            list(algSteps=10,niter=10,stopCr = 1e-04,simul=FALSE, verbose=FALSE)))
...
for (c in 1:simulation) {
  ...
  # Rand Index without bagging
  rand6.kmeans.iso[[c]]=RI(bagg6.kmeans.iso[[c]]$original$asig, verd6)
  
  
  # Rand Index with bagging
  rand.bagg6.kmeans.iso[[c]]=RI(max.col(as.matrix(bagg6.kmeans.iso[[c]]$bagg$.Data)), verd6)
  
  
  # FMI without bagging
  FM6.kmeans.iso[[c]]=FM_index_R(bagg6.kmeans.iso[[c]]$original$asig, verd6)
  
  
  
  # FMI with bagging
  FM.bagg6.kmeans.iso[[c]]=FM_index_R(max.col(as.matrix(bagg6.kmeans.iso[[c]]$bagg$.Data)), verd6)
  
  
  # Times
  
  time.without.bagg6[[c]]=bagg6.kmeans.iso[[c]]$tempo1
  
  time.with.bagg6[[c]]=bagg6.kmeans.iso[[c]]$tempo2
  
  
}

export.kmeans.iso6=cbind(rand6.kmeans.iso,rand.bagg6.kmeans.iso, FM6.kmeans.iso,  FM.bagg6.kmeans.iso, time.without.bagg6, time.with.bagg6, time.parallelization6)

write.table(export.kmeans.iso6, file = "kmeans_N100_iso6_B100_simul50.csv", sep = ",", 
            na = "", quote = FALSE, row.names = F)
...
parallep9=list()
Cluster9=list()
rand9.kmeans.iso=list()
rand.bagg9.kmeans.iso=list()
FM9.kmeans.iso=list()
FM.bagg9.kmeans.iso=list()
...
bagg9.kmeans.iso=parLapply(cl, Cluster9, function(x) cl_bag.LloydShapes(x, B=B, numClust=2, algorithm = "LloydShapes",  parameters = 
                                                                            list(algSteps=10,niter=10,stopCr = 1e-04,simul=FALSE, verbose=FALSE)))
...
for (c in 1:simulation) {
  ...
  # Rand Index without bagging
  rand9.kmeans.iso[[c]]=RI(bagg9.kmeans.iso[[c]]$original$asig, verd9)
  
  
  # Rand Index with bagging
  rand.bagg9.kmeans.iso[[c]]=RI(max.col(as.matrix(bagg9.kmeans.iso[[c]]$bagg$.Data)), verd9)
  
  
  
  # FMI without bagging
  FM9.kmeans.iso[[c]]=FM_index_R(bagg9.kmeans.iso[[c]]$original$asig, verd9)
  
  
  # FMI with bagging
  FM.bagg9.kmeans.iso[[c]]=FM_index_R(max.col(as.matrix(bagg9.kmeans.iso[[c]]$bagg$.Data)), verd9)
  
  # Times
  
  time.without.bagg9[[c]]=bagg9.kmeans.iso[[c]]$tempo1
  
  time.with.bagg9[[c]]=bagg9.kmeans.iso[[c]]$tempo2
  
  
}

export.kmeans.iso9=cbind(rand9.kmeans.iso,rand.bagg9.kmeans.iso, FM9.kmeans.iso,  FM.bagg9.kmeans.iso, time.without.bagg9, time.with.bagg9,time.parallelization9)

write.table(export.kmeans.iso9, file = "kmeans_N100_iso9_B100_simul50.csv", sep = ",", 
            na = "", quote = FALSE, row.names = F)
